...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        combined_mask = attention_mask if key_mask is None else np.logical_or(key_mask, attention_mask)
    ...
    def _split_heads(self, x):
        ...
        n_shape = x.shape[:-1] + (self.num_heads, self.embed_dim // self.num_heads)
    ...
    def _concat_heads(self, x):
        ...
        n_shape = x.shape[:-2] + (self.embed_dim,)
        x = x.reshape(n_shape)
