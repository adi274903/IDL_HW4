...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        pad_mask_src = PadMask.from_lengths(x_enc_lengths, x_enc.size(1)).to(x_enc.device)
    ...
    def decode(
        ...
        pad_mask_tgt = None
        if target_lengths is not None:
          pad_mask_tgt = PadMask.from_lengths(target_lengths, padded_targets.size(1)).to(padded_targets.device)
